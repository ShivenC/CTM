...
//Exporting function for handling the Update List Page
export default function UpdateListPage() {
  const [title, setTitle] = useState("");
  const [redirect, setRedirect] = useState("");
  const { subpage } = useParams([]);

  //Function tasked with Updating a list
  async function UpdateList(ev) {
    ev.preventDefault();
    try {
      await axios.patch("/updatelists/" + subpage, {
        title,
      });
      alert("List Added");
      setRedirect(true);
  ...
  //return elements responsible of data on screen for update list page
  return (
    <div className="mt-4 grow flex items-center justify-around">
      <div className="mb-60 w-96 border animate-fade animate-once animate-duration-[100ms] animate-normal rounded-xl border-stone-400 p-8 mt-16 py-20 bg-neutral-900">
        ...
        <form className="max-w-md mx-auto " onSubmit={UpdateList}>
          <input
            type="text"
