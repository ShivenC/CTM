...
//Exporting function for handling the Delete List Page
export default function DeleteList() {
  const [title] = useState("");
  const [redirect, setRedirect] = useState("");
  const { subpage } = useParams([]);

  //Function tasked with deleting a list
  async function DeleteList(ev) {
    ev.preventDefault();
    try {
      await axios.delete("/deletelists/" + subpage, {
        title,
      });
        ...
        <form className="max-w-md mx-auto ">
          <div className="flex">
            <div className="w-full flex items-center text-center justify-center">
              ...
              <button className="bg-stone-400" onClick={DeleteList}>
                Delete
              </button>
            </div>
          </div>
